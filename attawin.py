...
def normpath(path):
    ''' Returns the normalized path (doesn't change case). '''
    if path:
...
def get_used_drive():
    ''' Returns a list of drives used by a System, and subst.exe. '''
...
def get_drives_state():
    state = get_mount()
    state.extend([('%c' % drive, '') for drive in range(65, 91)
                  if not os.path.exists('%c:' % drive)])
    state.sort()
    return state
